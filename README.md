...
> A **"digital pandit"** skill for [Claude Code](https://claude.com/claude-code). It judges whether a decision is auspicious, picks an auspicious time (*muhurta*), checks Vastu, numerology, and planetary periods, and gives you a remedy (*upaya*) when something is off. In plain language, backed by real astronomy.
...
Ask it like you would ask a family pandit: *"good day to launch?"*, *"vastu for my factory?"*, *"am I in Sade Sati?"*, *"is this brand name lucky?"* It answers simply, with the heavy reasoning kept under the hood.
...
- 🔭 **Real ephemeris.** `panchang.py` and `kundli.py` use [pyephem](https://rhodesmill.org/pyephem/) (Lahiri sidereal) for tithi, nakshatra, **yoga, karana, Choghadiya, Abhijit, Rahu Kaal**, Mercury retrograde, **Vimshottari dasha**, and **Sade Sati**. No guessing.
- 🗣️ **Mentor, not textbook.** A one-line verdict plus two or three plain reasons. The depth lives in the engine, not the answer.
- 🔒 **Generic engine, private profile.** All the knowledge is reusable; personal birth data lives only in a **git-ignored `profile.md`**, and new users are **onboarded automatically**.
